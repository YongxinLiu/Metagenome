...
if (TRUE){
  ...
  if (opts$normalize==""){
    opts$normalize=paste(prefix, ".norm.txt", sep = "")}
  if (opts$output==""){
    opts$output=paste(prefix, ".alpha.txt", sep = "")}
...
# 保存统计结果，有waring正常
write.table(t(otu), file=opts$normalize, append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T)

# 3.2 保存alpha多样性指数
# 保存一个制表符，解决存在行名时，列名无法对齐的问题
write.table("Alpha_diversity\t", file=opts$output, append = F, quote = F, eol = "", row.names = F, col.names = F)
# 保存统计结果，有waring正常
write.table(alpha_div, file=opts$output, append = T, quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T)
