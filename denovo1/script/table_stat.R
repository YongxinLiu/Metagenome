...
if (TRUE){
  option_list <- list(
    make_option(c("-i", "--input"), type="character", default="temp/temp",
                help="OTU [default %default]"),
    make_option(c("-n", "--norm"), type="numeric", default=1000000,
                help="OTU [default %default]"),
    make_option(c("-o", "--output"), type="character", default="temp/temp1",
...
# colSums(df)
# 求和
df = as.data.frame(colSums(gene))

# 保存KO表
write.table("SampleID\tCounts", file=paste(opts$output, ".count", sep=""), append = F, sep="\t", quote=F, row.names=F, col.names=F)
write.table(df, file=paste(opts$output, ".count", sep = ""), append = T, sep="\t", quote=F, row.names=T, col.names=F)
